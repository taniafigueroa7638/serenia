...
async function initChatDatabase() {
  ...
  await query(`CREATE INDEX IF NOT EXISTS idx_chat_messages_conv_created ON chat_messages(conversation_id, created_at ASC)`);
  await query(`CREATE INDEX IF NOT EXISTS idx_chat_usage_date ON chat_usage_daily(usage_date)`);
}

module.exports = { initChatDatabase };
